...
import com.ditto.news.pipeline.NewsArticleCollector;
...
/**
 * 다중 RSS/Atom 피드로부터 K-컬처 뉴스 기사 후보를 수집, 중복 제거, 1차 키워드 필터링하는 서비스.
 * {@link NewsArticleCollector} 인터페이스를 구현하며, 개별 피드 장애 발생 시에도 전체 수집 파이프라인이
 * 중단되지 않고 정상 피드의 결과를 유지합니다.
 */
@Slf4j
@Service
    ...
    public List<NewsArticleCandidate> collectCandidates(List<RssFeedSource> sources, List<String> keywords) {
        ...
        for (RssFeedSource source : sources) {
            try {
                ...
                // 개별 피드 실패 시 경고 로그를 남기고 다음 피드로 계속 진행
